...
{
public:
	ChatRoom(int port, string name):TcpServer(port)
	{
		this->name = name;
	}
	virtual void handle_accept()
	{
		int fd = tcp_accept();
		clts.push_back(fd);
		string msg="Welcom to ChatRoom "+name+"!\n";
		tcp_write(fd, msg.c_str(), msg.size());
		string newcom = "A new client addin\n";
		broadcast(-1, newcom.c_str(), newcom.size());
        printf("Accetp fd = %d\n", fd);
	}
	virtual void handle_read_data(int fd)
	{
	...
	//广播消息
	void broadcast(int fd, const char *msg, int msg_len)
	{
		list<int>::iterator ite;
		for(ite = clts.begin(); ite != clts.end();	ite++)
			if(*ite != fd)
				tcp_asyn_write(*ite, msg, msg_len);
	}
